...
- **`ONBOARDING_EMAIL_ENABLED` gates the programme, and it ships off.** Added after the PR opened, because the external setup landed before the code did — merging and starting to mail every new signup had become the same action, and they should not be. It gates the ticker and the inline welcome send; it deliberately does **not** gate `send-test` (the point of merging switched off is being able to test it) or the Account invite (transactional, older than the programme). Verified in both states against the running stack: off is the default and a signup produces no attempt and no rows; on, the boot log says so, a signup reaches the send, and — with no ASM group locally — the claim is taken and correctly **released**, leaving `email_send` empty.
- **Before switching it on, consider the gap.** Anyone who signed up between deploy and flip is already past day 0/3/8, so the ticker walks them through the backlog one email a day, starting with a fortnight-late "Welcome to Big Shop". Same wound the launch cutoff exists to prevent, in miniature. If the gap has grown, `UPDATE email_launch SET launched_at = NOW() WHERE id = 1;` first.
