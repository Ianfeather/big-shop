...
// `Auth0Provider`'s `onRedirectCallback` - a second mechanism landing on the
// same navigation as the arrival logic in `pages/_app.tsx`'s `InnerApp`, which
// must wait for `POST /user` to create the account before anything routes
// anywhere (`hooks/use-account-setup.ts`). Two owners racing over one
// navigation is a bad trade for a delicate moment, and `lib/auth-callback.ts`
// documents just how delicate it is: Auth0 rewrites the URL with
// `history.replaceState` in a mount effect before any component renders.
// Storing the destination instead keeps a single owner, which simply chooses a
// better target than `/list` when there is one.
//
// Both ends of that live in `InnerApp`: it writes the value on the bounce and
// reads it back once the login resolves. An earlier draft consumed it in
// `pages/index.tsx`, which worked only while Auth0's callback landed on `/`;
// it goes straight to `/list` now (`lib/app-origin.ts`), so no single page is
// on every arrival's path any more.
//
