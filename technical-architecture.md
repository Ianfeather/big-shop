...
| `user` | One person, plus per-user flags: `onboarded` and `show_pantry_staples` (view preference — see CONTEXT.md's Pantry Staple). No longer the same thing as an Auth0 identity — see `user_identity` below |
| `user_identity` | Every Auth0 subject a person signs in with, aliased to one `user.id`. The indirection the multi-provider design rests on: two subjects can name one person, so signing in through a second provider does not mint a second, empty Account. The alias lives *above* `user` rather than beside it in `account_user`, because a great deal of shipped code assumes one human is one `user.id` — see `migrations/043_user_identity.sql` |
| `pending_link` | An account-link attempt in flight (token, `granted_subject`, `nonce_hash`, expiry). Written by `POST /link/start` and consumed by `POST /link/complete`; a row, rather than a signed stateless token, so that redemption can be genuinely single-use. Purged lazily on the write path — deliberately not on the redemption path, where it would mask an expiry. See `specs/completed/account-linking-recovery.md` |
...
| `TIDB_USER` | none — required | Same place. Several accounts use these variables and they are deliberately not interchangeable: the API connects as `<prefix>.api` ([four DML privileges on `bigshop`](./docs/api-database-user.md)), the deploy's migration step as `<prefix>.gh_migrate` ([DDL as well](./docs/ci-database-user.md)), the three read-only scripts in `scripts/` as `<prefix>.reporting` ([`SELECT` and nothing else](./docs/reporting-database-user.md)), and `backup-prod.sh`/`probe-charset-conversion.sh` as root, which they need. `scripts/lib/tidb-env.sh` picks between them: an exported `TIDB_USER` always wins, otherwise a script asks for its own role's variable (`TIDB_MIGRATE_USER`, `TIDB_READONLY_USER`) and falls back to `TIDB_USER` |
