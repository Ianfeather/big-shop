...
| `pending_link` | An account-link attempt in flight (token, `granted_subject`, `nonce_hash`, expiry). Written by `POST /link/start` and consumed by `POST /link/complete`; a row, rather than a signed stateless token, so that redemption can be genuinely single-use. Purged lazily on the write path — deliberately not on the redemption path, where it would mask an expiry. See `specs/account-linking-recovery.md` |
