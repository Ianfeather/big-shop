...
import { recordTokenUsage } from '../telemetry/metrics';
import { llmSpan } from '../telemetry/llm-span';
...
export async function extractRecipe({ input, knownIngredients = [], knownUnits = [] }) {
  ...
  // Spanned and counted here rather than at each of the three routes that call
  // it, because this is the call - the one that takes seconds and costs money.
  // A route's own span already says which Import Source asked; this says what
  // the asking actually cost.
  const response = await llmSpan(EXTRACTION_MODEL, 'extract_recipe', async () => {
    ...
    recordTokenUsage(EXTRACTION_MODEL, r.usage);
    return r;
  });
...
export async function extractMethod({ input }) {
  ...
  const response = await llmSpan(EXTRACTION_MODEL, 'extract_method', async () => {
    const r = await openai.responses.create({
      model: EXTRACTION_MODEL,
      input: buildRequestInput(input, instructions),
      text: {
